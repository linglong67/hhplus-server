...
- ``만료 대상 얘약 티켓의 좌석 조회`` (findAllByReservationIdIn)
    ```sql
    select
        rte1_0.id,
...
- ``콘서트 예약가능 좌석 조회`` → **단일 인덱스 사용**
    ...
    [Explain Analyze 결과]  
    -> Limit: 200 row(s)  (cost=25.3 rows=23) (actual time=0.129..0.137 rows=23 loops=1)
        -> Index lookup on cse1_0 using idx_schedule_id_status (concert_schedule_id=100000, status='AVAILABLE')  (cost=25.3 rows=23) (actual time=0.0931..0.1 rows=23 loops=1)
